# Claude Hook Guard — WSL bridge

WSL-side components for Claude Hook Guard v1. When Claude Code (running in WSL) is about to execute a tool, a PreToolUse hook forwards the event to the Windows-side Tauri app over a WebSocket and blocks until the user approves or denies it.

## Contents

- `pre-tool-use.sh` — bash hook script Claude Code invokes. Pipes stdin to `bridge.mjs`, exits `0` on `approve` and `1` on `deny`.
- `bridge.mjs` — ESM Node script that connects to `ws://localhost:9876`, sends a `hook_event`, awaits a matching `hook_response`, and prints `approve`/`deny` on stdout.
...
Make sure the Windows Tauri app is running (listening on `ws://localhost:9876`), then:
...
- **Hook always denies / "connection failed"** — the Windows Tauri app isn't running. Start it so it can listen on `ws://localhost:9876`.
